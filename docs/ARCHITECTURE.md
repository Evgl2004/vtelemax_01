...
10. Во всех адаптерах вход в режим модератора выполняется только командой `/mod`.
11. Доступ к `/mod` проверяется по признаку `persons.is_moderator` в БД (без отдельной admin-роли).
12. В Telegram/VK/MAX восстановлен сценарий `/mod` с FSM-меню модератора (список тикетов, ответ, карточка).
