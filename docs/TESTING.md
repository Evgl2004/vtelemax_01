...
26. Переходная авторизация iiko:
   1. выбор `v1` по умолчанию и проверка полного набора `v2`;
   2. сохранение запуска приложений при неполной конфигурации;
   3. точные адреса и тела запросов `v1`/`v2`;
   4. отсутствие автоматического возврата с `v2` на `v1`;
   5. передача настроек во все пять точек сборки и отдельная диагностика `v2`.
