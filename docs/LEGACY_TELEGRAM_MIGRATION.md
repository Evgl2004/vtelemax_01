...
## 5. Рассылка приветственных сообщений

После миграции данных legacy-пользователей необходимо уведомить их о переходе на новый бот и очистить кэш старых reply-кнопок. Для этого предназначен скрипт `scripts/legacy_telegram_broadcast.py`.

### Цели рассылки

1. **Очистка кэша reply-кнопок** – отправка `ReplyKeyboardRemove()` каждому пользователю, чтобы удалить старые клавиатуры из старого бота.
2. **Отправка приветственного сообщения** – текст содержит:
   - объяснение перехода на обновлённую систему;
   - просьбу ввести команду `/start` для активации нового меню;
   - краткое описание структуры основного меню бота;
   - ссылки на альтернативные боты (VK, MAX);
   - оформление с эмодзи и корректными гиперссылками.

### Запуск рассылки

Скрипт поддерживает те же аргументы, что и миграция:

```powershell
# Предпросмотр получателей (без отправки)
.\.venv\Scripts\python.exe scripts/legacy_telegram_broadcast.py --dry-run

# Точечная рассылка одному номеру (для тестов)
.\.venv\Scripts\python.exe scripts/legacy_telegram_broadcast.py --phone +79129923438 --dry-run

# Фактическая рассылка с задержкой 0.5 сек между отправками
.\.venv\Scripts\python.exe scripts/legacy_telegram_broadcast.py --yes --delay 0.5

# Ограничение количества обрабатываемых записей
.\.venv\Scripts\python.exe scripts/legacy_telegram_broadcast.py --yes --limit 100 --offset 0
```

### Ключевые параметры

- `--source-db` – путь к SQLite-источнику (по умолчанию из `LEGACY_SOURCE_SQLITE_PATH`).
- `--phone` – фильтр по номеру для точечной отправки.
- `--delay` – задержка между отправками в секундах (по умолчанию 0.5).
- `--dry-run` – только предпросмотр получателей и текста сообщения.
- `--yes` – подтверждение фактической отправки.
- `--limit`, `--offset` – пагинация source-записей.

### Логика работы

1. Чтение записей из source SQLite (таблица `user_phones`).
2. Отбор уникальных `telegram_user_id` с фильтрацией по номеру (если задан).
3. Для каждого пользователя:
   - отправляется `ReplyKeyboardRemove()` для очистки кэша reply-кнопок;
   - отправляется приветственное сообщение.
4. Соблюдение лимитов Telegram – между отправками выдерживается задержка (`--delay`).
5. Обработка ошибок:
   - `TelegramForbiddenError` – бот заблокирован пользователем (пропуск);
   - `TelegramRetryAfter` – превышен лимит запросов (ожидание и пропуск пользователя);
   - `TelegramBadRequest` – прочие ошибки API (логирование, продолжение).
6. По окончании печатается сводка: количество успешных/неудачных отправок.

### Текст сообщения

Сообщение формируется функцией `build_default_legacy_broadcast_message()` из модуля `vtelemax.tools.legacy_telegram_broadcast`. Текст можно адаптировать, изменив эту функцию.

### Интеграция с миграцией

Рекомендуемый порядок действий:

1. Выполнить миграцию данных (`scripts/migrate_legacy_telegram_users.py --yes`).
2. Запустить рассылку (`scripts/legacy_telegram_broadcast.py --yes --delay 0.5`).
3. После рассылки можно запускать нового бота – пользователи будут готовы к работе с новым меню.

## 6. Точки развития (зафиксировано)
