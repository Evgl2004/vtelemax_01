...
24. Добавлена временная поддержка авторизации iiko `v1`/`v2` с управляемым
    переключением через `IIKO_AUTH_VERSION`.
...
```dotenv
# По умолчанию используется совместимая авторизация v1.
IIKO_AUTH_VERSION=v1
IIKO_API_KEY=...
IIKO_ORG_ID=...

# Для проверки и последующего включения v2 добавьте новые реквизиты:
IIKO_APP_ID=...
IIKO_CLIENT_SECRET=...
IIKO_CLOUD_API_KEY=...

# Адреса обычно менять не требуется.
IIKO_AUTH_URL=https://api-ru.iiko.services/api/v2/access_token
IIKO_BASE_URL=https://api-ru.iiko.services/api/1
```

Для включения новой авторизации измените только `IIKO_AUTH_VERSION=v2` и
перезапустите процессы. Рабочие методы лояльности при этом остаются на `/api/1`.
...
5. Для включения функционала лояльности заполните набор выбранной версии авторизации
   iiko и `IIKO_ORG_ID`. При неполном наборе приложение продолжит работу без шлюза
   iiko и покажет сообщение о недоступности соответствующих функций.
...
10. Переход на новую авторизацию iiko:
    `docs/IIKO_CLOUD_API_V2_AUTH_MIGRATION_ANALYSIS.md`.
