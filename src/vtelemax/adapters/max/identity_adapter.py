...
class MaxIdentityAdapter:
    ...
    def handle_start(self, max_user_id: int) -> MaxAdapterResponse:
        ...
        platform_consents_complete = (
            ...
            and person.get_notifications_allowed_at_for_platform("max") is not None
        )
    ...
    def _open_profile_notifications_edit(self, *, max_user_id: int) -> MaxAdapterResponse:
        ...
        notifications_allowed = person.get_notifications_allowed_for_platform("max")
        self._state_by_user_id[max_user_id] = _STATE_PROFILE_EDIT_NOTIFICATIONS
    ...
    def _toggle_profile_notifications(
        ...
        *,
        ...
        """Переключает/включает уведомления для MAX-платформы в профиле."""
        ...
        current_value = person.get_notifications_allowed_for_platform("max")
        ...
        if target_value == current_value:
            ...
            return MaxAdapterResponse(
                text=f"ℹ️ Статус уведомлений уже: {status_text}.\n\n{profile.text}",
    ...
    def _handle_moderator_menu_choice(self, *, max_user_id: int, lowered_text: str) -> MaxAdapterResponse:
        ...
        if lowered_text in {"7", "вработу", "в_работу", "take"}:
            self._moderator_state_by_user_id[max_user_id] = _STATE_MOD_WAIT_TICKET_FOR_IN_PROGRESS
            self._moderator_context_by_user_id.pop(max_user_id, None)
            return MaxAdapterResponse(
                text=(
    ...
    def _is_moderator_account(self, *, max_user_id: int) -> bool:
    ...
    def _handle_action(self, max_user_id: int, action: GuestMenuAction) -> MaxAdapterResponse:
        ...
        if action == GuestMenuAction.PROFILE_NOTIFICATIONS_ENABLE:
            return self._toggle_profile_notifications(max_user_id=max_user_id, new_value=True)

        if action == GuestMenuAction.PROFILE_NOTIFICATIONS_TOGGLE:
            return self._toggle_profile_notifications(max_user_id=max_user_id, new_value=None)
    ...
    def _handle_balance_action(self, *, max_user_id: int, person_phone_e164: str) -> MaxAdapterResponse:
    ...
    def _handle_virtual_card_action(self, *, max_user_id: int, person_phone_e164: str) -> MaxAdapterResponse:
    ...
    def _create_external_error_ticket_for_guest(
        ...
        *,
        ...
        method_logger = self._logger.bind(stage="external_error_ticket", user_id=str(max_user_id))
        try:
            self._create_support_ticket_use_case.execute(
                CreateSupportTicketCommand(
                    platform="max",
    ...
    def _format_person_tickets_message(tickets: tuple[PersonSupportTicketSummary, ...]) -> str:
        ...
        for i, ticket in enumerate(tickets, 1):
            ...
            short_id = MaxIdentityAdapter._format_ticket_id_short(ticket.ticket_id)
            lines.append(
