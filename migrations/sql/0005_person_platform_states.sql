...
SET
    rules_accepted = CASE
        WHEN EXCLUDED.rules_accepted = TRUE THEN TRUE
        ELSE person_platform_states.rules_accepted
    END,
    rules_accepted_at = COALESCE(
        person_platform_states.rules_accepted_at,
        EXCLUDED.rules_accepted_at
    ),
    notifications_allowed = CASE
        WHEN EXCLUDED.notifications_allowed = TRUE THEN TRUE
        ELSE person_platform_states.notifications_allowed
    END,
    notifications_allowed_at = COALESCE(
        person_platform_states.notifications_allowed_at,
        EXCLUDED.notifications_allowed_at
    ),
    is_registered = CASE
        WHEN EXCLUDED.is_registered = TRUE THEN TRUE
        ELSE person_platform_states.is_registered
    END,
    registered_at = COALESCE(
        person_platform_states.registered_at,
